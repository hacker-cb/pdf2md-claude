...
- `pdf2md_claude/cli.py` -- Entry point. Parses args, creates `PdfConverter` and `ConversionPipeline`, iterates PDFs, delegates each to `pipeline.convert()` / `pipeline.remerge()`, prints summary.
- `pdf2md_claude/pipeline.py` -- Single-document orchestration via `ConversionPipeline` class. Holds image settings; `convert()` creates work directory, calls `PdfConverter.convert()`, merges chunks, validates, writes output. `remerge()` re-runs merge+validate from cached chunks. Shared post-processing is in `_post_process()`. Key types: `ConversionPipeline`, `PipelineResult`.

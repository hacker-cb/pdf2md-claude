...
- `pdf2md_claude/workdir.py` -- Chunk persistence and resume. Manages a `.chunks/` directory with manifest-based staleness detection. All cross-chunk data flows through disk (never in memory). `load_manifest()` provides lenient manifest reading (returns `None` on missing/corrupt). Key types: `Manifest`, `ChunkUsageStats`, `WorkDir`.

...
<!-- IMAGE_BEGIN -->
<!-- IMAGE_RECT 0.05,0.04,0.95,0.25 -->
...
![Figure 1: Sketch of Docling's default processing pipeline. The inner part of the model pipeline is easily customizable and extensible.](docling.images/img_p003_01.png)
<!-- IMAGE_AI_GENERATED_DESCRIPTION_BEGIN -->
> The diagram shows a linear processing pipeline flowing from left to right. It begins with a PDF document icon labeled "Parse PDF pages" on the left. Arrows lead to three processing steps within a dashed-bordered box labeled "Model Pipeline": "OCR", "Layout Analysis", and "Table Structure". Above the model pipeline, document page icons illustrate the flow of pages being processed. After the model pipeline, arrows lead to "Assemble results, Apply document post-processing" and finally to "Serialize as JSON or Markdown" represented by a {;} icon on the far right.
...
<!-- IMAGE_BEGIN -->
<!-- IMAGE_RECT 0.07,0.06,0.93,0.82 -->
...
![Figure 2: Title page of the DocLayNet paper (arxiv.org/pdf/2206.01062) - left PDF, right rendered Markdown. If recognized, metadata such as authors are appearing first under the title. Text content inside figures is currently dropped, the caption is retained and linked to the figure in the JSON representation (not shown).](docling.images/img_p007_01.png)
<!-- IMAGE_AI_GENERATED_DESCRIPTION_BEGIN -->
> The figure shows a side-by-side comparison of the first page of the DocLayNet paper. On the left is the original PDF rendering showing the paper's title "DocLayNet: A Large Human-Annotated Dataset for Document-Layout Analysis" with author names, affiliations, abstract, CCS concepts, keywords, ACM reference format, introduction section, and a Figure 1 containing four examples of complex page layouts across different document categories (including scientific articles, product manuals, FAA charts, financial reports, and driving guides). On the right is the rendered Markdown output from Docling, showing the same content converted to structured Markdown format. The Markdown version shows the title as a heading, followed by author metadata (names, affiliations, emails listed individually), the abstract text, CCS concepts, keywords, ACM reference format, Figure 1 caption, and the beginning of the introduction. The figure demonstrates how Docling preserves document structure while converting from PDF to Markdown.
<!-- IMAGE_AI_GENERATED_DESCRIPTION_END -->
<!-- IMAGE_END -->
...
<!-- IMAGE_BEGIN -->
<!-- IMAGE_RECT 0.07,0.03,0.93,0.80 -->
...
![Figure 3: Page 6 of the DocLayNet paper. If recognized, metadata such as authors are appearing first under the title. Elements recognized as page headers or footers are suppressed in Markdown to deliver uninterrupted content in reading order. Tables are inserted in reading order. The paragraph in "5. Experiments" wrapping over the column end is broken up in two and interrupted by the table.](docling.images/img_p008_01.png)
<!-- IMAGE_AI_GENERATED_DESCRIPTION_BEGIN -->
> The figure shows a side-by-side comparison of page 6 of the DocLayNet paper. On the left is the original PDF rendering showing a two-column academic paper layout with Table 2 (Prediction performance mAP@0.5-0.95 of object detection networks on DocLayNet test set, comparing human, MRCNN R50, MRCNN R101, FRCNN R101, and YOLO v5x6 across 11 document element classes), Figure 5 (a line graph showing prediction performance of Mask R-CNN with ResNet50 backbone trained on increasing fractions of DocLayNet dataset), text about baselines for object detection, and section 5 "Experiments". On the right is the rendered Markdown output from Docling, showing the same content converted to structured text format. The Markdown preserves the table data in a grid format, includes the figure caption, and renders the body text in reading order. Page headers and footers from the original are suppressed in the Markdown output.
<!-- IMAGE_AI_GENERATED_DESCRIPTION_END -->
<!-- IMAGE_END -->
...
<!-- IMAGE_BEGIN -->
<!-- IMAGE_RECT 0.05,0.03,0.95,0.82 -->
...
![Figure 4: Table 1 from the DocLayNet paper in the original PDF (A), as rendered Markdown (B) and in JSON representation (C). Spanning table cells, such as the multi-column header "triple inter-annotator mAP@0.5-0.95 (%)", is repeated for each column in the Markdown representation (B), which guarantees that every data point can be traced back to row and column headings only by its grid coordinates in the table. In the JSON representation, the span information is reflected in the fields of each table cell (C).](docling.images/img_p009_01.png)
<!-- IMAGE_AI_GENERATED_DESCRIPTION_BEGIN -->
> The figure contains three panels labeled A, B, and C. Panel A (top-left) shows the original PDF rendering of Table 1 from the DocLayNet paper, which is a detailed dataset overview table with columns for class label, Count, % of Total (Train, Test, Val), and triple inter-annotator mAP @ 0.5-0.95 (%) broken down by All, Fin, Man, Sci, Law, Pat, Ten categories. The table lists 11 class labels (Caption, Footnote, Formula, List-item, Page-footer, Page-header, Picture, Section-header, Table, Text, Title) plus a Total row. Panel B (top-right) shows the same table as rendered Markdown output from Docling, where multi-column spanning headers are repeated for each column to ensure every data point can be traced to its row and column headings by grid coordinates alone. Panel C (bottom-left) shows a JSON code snippet representing part of the table's data structure, with fields including "bbox" (bounding box coordinates), "spans", "text" ("triple inter-annotator mAP @ 0.5-0.95 (%)"), "col": 5, "col-span": [5,12], "row": 0, and "row-span": [0,1], demonstrating how cell span information is encoded in the JSON format.
<!-- IMAGE_AI_GENERATED_DESCRIPTION_END -->
<!-- IMAGE_END -->
